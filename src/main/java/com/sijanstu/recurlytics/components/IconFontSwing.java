...
final class IconFontSwing {
    ...
    public static synchronized void register(IconFont iconFont) {
        if (iconFont != null && !IconFontSwing.fonts.contains(iconFont)) {
    ...
    public static synchronized final Font buildFont(String fontFamily) {
        try {
            ...
            Logger.getAnonymousLogger().log(Level.SEVERE, "Font load failure", ex);
        }

        Logger.getAnonymousLogger().log(Level.SEVERE, "Font not found: {0}", fontFamily);
